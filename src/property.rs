...
impl PropertyBag {
    ...
    pub fn get(&self, id: PropertyId) -> Result<Option<String>> {
        unsafe {
    ...
    pub fn set<T: AsRef<str>>(&mut self, id: PropertyId, v: T) -> Result<()> {
        let s = CString::new(v.as_ref())?;
        unsafe {
            convert_err(
                property_bag_set_string(self.get_handle(), id as i32, NULL_C_STR_PTR, s.as_ptr()),
                "PropertyBag.set error",
            )?;
        }
        Ok(())
    }
